...
class Dashboard extends PureComponent {
  constructor(props) {
    ...
    let clientName;
    if(loggedUser && loggedUser.clientId)  clientName=loggedUser.clientId.name;
    this.state = {
      checked: true,
      dashboardData: {},
      ...
      columns: [
        ...
        { key: 'CreatedByUserId.name', name: 'Created By', width: 150, filter: false, sort: true, type: 'string', formatter: (props, record) => record && record.CreatedByUserId ? record.CreatedByUserId.firstName + ' ' + record.CreatedByUserId.lastName : "System" },
  ...
  componentWillMount() {
    ...
    let user = utils.getLoggedUser();
    console.log('userrrrrrrr2466', user)
    if (user) {
  ...
  onLayoutChange = (currentLayout, allLayouts) => {
    ...
    this.setState({ layoutConfigurations: allLayouts });
  }
  ...
  render() {
    ...
    const { dashboardData, dashboardWidgets, dashboardConfigs, isOpen, selectedValues, layoutConfigurations, checked, suspiciousTransaction, suspiciousTransactionOpen, updateDiv, selectedOption, selectedCameraOption, isOpenPendingClip, columns, clientName } = this.state,
    ...
    return (
      <div className="animated fadeIn" id="dashboardBody">
        <Row>
          <LoadingDialog isOpen={isFetching || !updateDiv} />
          {
            // localStorage.getItem('user_client') == "Spirit ATL" ? <DashboardALT/>
            clientName == "Spirit ATL" ? <DashboardALT/>
        ...
        {
          // localStorage.getItem('user_client') != "Spirit ATL" ?
          clientName != "Spirit ATL" ?
        <Card>
          ...
          <CardBody className="dashboardMain-body" style={{ minHeight: 684 }}>
            {dashboardConfigs.length > 0 ?
              updateDiv && <ResponsiveGridLayout className="layout custom-grid-body" style={{marginRight:150}}
              ...
              >
                {dashboardConfigs.map(dashboardConfig => {
                  ...
                  if (hasPermission) {
                    switch (dashboardConfig.label) {
                      ...
                      case "Suspicious Transactions":
                        ...
                        component = <SuspiciousTransaction checkedStatus={this.setCheckedFalse} data={dashboardData} onDataLoaded={this.onDataLoaded} {...this.props} />
                        break;
                      case "Map":
                        component = <div style={{ width: '100%', height: '100%', minHeight: '300px' }}><MapView /></div>
                        break;
                      case "Latest Events":
                        component = <TopEvent checkedStatus={this.setCheckedFalse} data={dashboardData} {...this.props} isFromDashboard />
                        break;
                      case "Video Clips":
                        component = <TopVideoClips checkedStatus={this.setCheckedFalse} data={dashboardData} {...this.props} isFromDashboard />
                        break;
                      case "People Counting":
                  ...
                  let latestVideoClips = dashboardConfig.label == 'Video Clips';
                  let topSellers = dashboardConfig.label == 'Top Selling Items';
                  return (
...
var DashboardModule = connect(mapStateToProps)(Dashboard);

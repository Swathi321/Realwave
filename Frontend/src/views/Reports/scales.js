...
export class Scales extends PureComponent {
    constructor(props) {
        super(props)
        ...
        let Columns = [
            ...
            // {
            //     key: 'DateTime', name: 'Date Time', width: 250, sort: true, type: 'string', formatter: (props, record) => {
            //         var timezone = moment.tz.guess();
            //         return (
            //             moment(moment(record.DateTime)).tz(timezone).format(utils.dateTimeFormatAmPm)
            //         )
            //     }
            // },
            {
                key: 'Weight', filter: true, name: 'Weight (Lbs)', width: 150, align: 'right', sort: true, type: 'numeric', formatter: (props, record) => {
                    let color = record && record.StoreId && record.StoreId.color ? "#000" : "inherit";
                    return (
                        <div style={{ background: record.StoreId.color, color: color }}>{record.Weight}</div>
                    )
                }
            },
            { key: 'ScaleId', name: 'Scale Name', type: 'scaleId', filter: true, width: 200, sort: true },
            { key: 'CamId', name: 'Camera', type: 'camId', filter: true, width: 200, sort: true },

            // {
            //     key: 'VideoClipId ? VideoClipId.IsVideoAvailable : VideoClipId', name: 'Clip', width: 250, toggle: true, filter: true, sort: true, type: 'String',
            //     formatter: (props, record) => record.VideoClipId && record.VideoClipId.IsVideoAvailable ? <div >
            //     <PlayCircleOutlined />
            //     </div> :
            //     <div>
            //         <MinusCircleOutlined />
    ...
    componentWillReceiveProps(nextProps) { // NOTE component will receive props
        ...
        //     console.log("🚀 ~ file: scales.js ~ line 161 ~ Scales ~ componentWillReceiveProps ~ nextProps['getReceiptClip']", nextProps['getReceiptClip'])
        //     let valid = common.responseHandler(data, error, isFetching);
        //     if (valid) {
    ...
    render() {
        const { columns, loadedData, ShowAddBtn, dataSource, isOpen, currentReceipt, currentRecord, isGridView, appliedTheme } = this.state;
...
var ScalesModule = connect(mapStateToProps)(Scales);
